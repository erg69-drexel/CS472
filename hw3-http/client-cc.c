...
int process_request(const char *host, uint16_t port, char *resource){
    ...
    // generate the request with helper
    char *request = generate_cc_request(host, port, resource);

    //get size of request
    size_t req_size = strlen(request);

    //send request, but need to loop around to ensure full send.

    int bytes_sent = 0;

    while(bytes_sent < req_size){
        int send_ret = send(sock, request + bytes_sent, req_size - bytes_sent, 0);
        if (send_ret < 0){
