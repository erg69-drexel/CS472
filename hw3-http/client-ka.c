...
int submit_request(int sock, const char *host, uint16_t port, char *resource){
    ...
    // from the server
    //
    // YOUR ANSWER:  For the first line, initial_data, we are looking at the length of the data, 
    // excluding the header up to this point, as demonstrated by the formula, bytes_recvd (bytes we have gotten so far),
    // minus the length of the header. Doing this leaves us with the initial size of the actual data/payload/body.

    //The second line, bytes_remaining, tells us how many additional bytes we need to recieve to get
    // the full payload/body. It subtracts the total content length from the amount we currently have at this point.
    //
    //--------------------------------------------------------------------------------
    int initial_data =  bytes_recvd - header_len;
    printf("Initial Data Length: %d\n", initial_data);
    ...
    while(bytes_remaining > 0){
        ...
        //need to make sure bytes remaining fits into buffer:
        int bytes_to_send = 0;
        if (bytes_remaining > BUFF_SZ){
            bytes_to_send = BUFF_SZ;
        } else{
