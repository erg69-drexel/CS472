...
void get_current_ntp_time(ntp_timestamp_t *ntp_ts){
    ...
    */
    ...
    ntp_ts->fraction = ((tv.tv_usec * NTP_FRACTION_SCALE) / USEC_INCREMENTS);
...
void ntp_time_to_string(const ntp_timestamp_t *ntp_ts, char *buffer, size_t buffer_size, int local) {
    ...
    // Hint: Convert NTP to Unix time, use localtime/gmtime, format with snprintf
         //1. Convert NTP timestamp back to Unix time (subtract NTP_EPOCH_OFFSET)
     time_t unix_sec = NTP_TO_UNIX_SECONDS(ntp_ts->seconds);
     // 2. Convert NTP fraction back to microseconds
     suseconds_t usec = (suseconds_t)ntp_ts->fraction * USEC_INCREMENTS / NTP_FRACTION_SCALE;
    //3. Use localtime() or gmtime() based on 'local' parameter
    struct tm *tm_ptr;
    if (local == UTC_TIME){
        tm_ptr = gmtime(&unix_sec);
    }else{
        tm_ptr = localtime(&unix_sec);
    }

    if (tm_ptr == NULL){
        snprintf(buffer, buffer_size, "INVALID_TIME");
    }else{
        snprintf(buffer, buffer_size, "%04d-%02d-%02d %02d:%02d:%02d.%06d", 
        tm_ptr->tm_year + NTP_EPOCH_YEAR,
        tm_ptr->tm_mon + 1,
        tm_ptr->tm_mday,
        tm_ptr->tm_hour,
        tm_ptr->tm_min,
...
double ntp_time_to_double(const ntp_timestamp_t* timestamp) {
    ...
    double fraction = (double) timestamp->fraction / (double) NTP_FRACTION_SCALE;
    double total = seconds + fraction;
    return total;
}
...
void print_ntp_time(const ntp_timestamp_t *ts, const char* label, int local){
    ...
    ntp_time_to_string(ts, time_string, sizeof(time_string), local);

    if (local == UTC_TIME){
        printf("%s: %s (UTC)\n", label, time_string);
    } else {
        printf("%s: %s (Local Time)\n", label, time_string);
    }
}
...
int decode_reference_id(uint8_t stratum, uint32_t ref_id, char *buff, int buff_sz){
    ...
    if (stratum >= 2){
        ...
        //convert to htonl
        uint32_t conv = htonl(ref_id);

        //use inet_ntop()

        inet_ntop(AF_INET, &conv, buff, buff_sz);
...
void print_ntp_packet_info(const ntp_packet_t* packet, const char* label, int packet_type) {
    ...
    //4. Decode and print reference_id using decode_reference_id()
    char buff[16];
    decode_reference_id(packet->stratum, packet->reference_id, buff, sizeof(buff));
    ...
    //check packet type to ensure correct printing of the times
    if (packet_type == 1){
        ...
        print_ntp_time(&packet->xmit_time, "Transmit (T3) Time", 1);
    } else{
        printf("Reference time: NOT SET\n");
        printf("Orginal Time (T1): NOT SET\n");
        printf("Receive Time (T2): NOT SET\n");
        print_ntp_time(&packet->xmit_time, "Transmit (T3) Time", 1);
    }
...
void print_ntp_results(const ntp_result_t* result) {
    ...
    if (result->offset > 0.0){
        printf("Your click is running BEHIND by %.02f ms\n", (result->offset * 1000));
    } else{
        printf("Your click is running AHEAD by %.02f ms\n", (result->offset * 1000));
    }
    
    printf("Your estimated time error will be +/- %.02f ms\n", (result->final_dispersion * 1000));
}